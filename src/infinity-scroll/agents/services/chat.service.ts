...
import { narrators } from '../girlfriends/girlfriends';
...
export class ChatService {
  ...
  private async getMemoryForUser(userId: string, girlfriendId: string): Promise<BufferMemory> {
    ...
    if (memoryData?.history) {
      for (const message of memoryData.history) {
        const MessageClass = message.type === 'human'
          ? HumanMessage
          : message.type === 'ai'
          ? AIMessage
          : SystemMessage;

        chatHistory.addMessage(new MessageClass(message.content));
      }
    }

    return new BufferMemory({ memoryKey: 'history', returnMessages: true, chatHistory  ,   outputKey: "output",});
  }
  ...
  private async saveMemory(userId: string, girlfriendId: string, memory: BufferMemory): Promise<void> {
    ...
    await this.memoryModel.findOneAndUpdate(
      { userId, girlfriendId },
      { userId, girlfriendId, history: storableHistory, updatedAt: new Date() },
      { upsert: true }
    );
  }
